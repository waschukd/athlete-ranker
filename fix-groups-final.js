...
// Remove ALL existing export function blocks
c = c.replace(/\n\s*const currentSession[\s\S]*?setTimeout\(\(\) => w\.print\(\), 500\);\s*\};\n/g, '\n');

// Remove duplicate icon imports
c = c.replace(', Download, Printer, Download, Printer', ', Download, Printer');
c = c.replace(', Download, Printer', '');
c = c.replace('RefreshCw', 'RefreshCw, Download, Printer');

// Remove any existing export buttons
c = c.replace(/\s*\{groups\.length > 0 && assignments\.length > 0 && \(<>[\s\S]*?<\/>\)\}/g, '');

// Inject functions once after goalies line
c = c.replace(
