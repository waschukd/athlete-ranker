...
//
// Line pricing mirrors /api/evaluator/pay and the SP payroll route exactly:
// an hour is TESTING or EVALUATION by the session it was logged against, and
// pays at tester_hourly_rate or hourly_rate from the membership with THIS
// provider — never the org that happened to run the session.
...
async function buildInvoice(userId, orgId) {
  const [em] = await sql`
...
export async function POST(request) {
  try {
    ...
    const invoiceNo = `INV-${userId}-${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, "0")}${String(today.getDate()).padStart(2, "0")}`;
    ...
    const res = await sendEmail(to, `Invoice ${invoiceNo} — ${inv.em.user_name} — ${money(inv.total)}`, emailWrapper(content));
