...
export async function GET(request) {
  try {
    ...
    // Get all evaluators in this SP's pool with stats
    const evaluators = await sql`
      SELECT 
        ...
        -- Hours only credited when evaluator actually scored (evaluator_hours created on first score)
      ...
      LEFT JOIN evaluator_hours eh ON eh.evaluator_id = u.id 
        AND eh.schedule_id = ess.schedule_id
        AND eh.organization_id IN (
          SELECT association_id FROM sp_association_links WHERE service_provider_id = ${spId} AND status = 'active'
        )
    ...
    const flags = await sql`
      SELECT ef.*, u.name as evaluator_name, es.session_number, es.scheduled_date,
        o.name as org_name
    ...
    const pendingHours = await sql`
      SELECT eh.*, u.name as evaluator_name, 
        es.session_number, es.scheduled_date, es.start_time, es.end_time,
...
export async function POST(request) {
  try {
    ...
    if (action === "approve_hours") {
      await sql`
        UPDATE evaluator_hours SET status = 'approved', approved_by = ${admin_id}, approved_at = NOW()
        WHERE id = ${hours_id}
      `;
    ...
    if (action === "rate_evaluator") {
      await sql`
        INSERT INTO evaluator_ratings (evaluator_id, rated_by, organization_id, schedule_id, rating, notes)
        VALUES (${evaluator_id}, ${admin_id}, ${sp_id}, ${schedule_id}, ${rating}, ${notes || null})
        ON CONFLICT (evaluator_id, schedule_id) DO UPDATE SET rating = ${rating}, notes = ${notes || null}
      `;
      return NextResponse.json({ success: true });
    }
    ...
    if (action === "approve") {
      await sql`
        UPDATE evaluator_memberships SET status = 'active', pending = false
        WHERE user_id = ${evaluator_id} AND organization_id = ${sp_id}
      `;
      await sql`
        INSERT INTO audit_log (user_id, action, entity_type, entity_id, new_value)
        VALUES (${admin_id}, 'evaluator_approved', 'user', ${evaluator_id}, 'approved by SP admin')
      `;
      // Notify the evaluator
      const evalUser = await sql`SELECT email, name FROM users WHERE id = ${evaluator_id}`;
      const spOrg = await sql`SELECT name FROM organizations WHERE id = ${sp_id}`;
      if (evalUser.length && process.env.RESEND_API_KEY) {
        await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: evalUser[0].email,
            subject: `✅ You've been approved — ${spOrg[0]?.name}`,
            html: `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
              <h2>You're approved!</h2>
              <p>Hi ${evalUser[0].name}, you've been approved as an evaluator for <strong>${spOrg[0]?.name}</strong>.</p>
              <p>Sign in to start picking up sessions.</p>
    ...
    if (action === "suspend") {
      // Suspend — keep all data, just block access
      await sql`
        UPDATE evaluator_memberships SET status = 'suspended'
        WHERE user_id = ${evaluator_id} AND organization_id = ${sp_id}
      `;
      // Remove from all future sessions but keep history
      await sql`
      ...
      await sql`
        INSERT INTO audit_log (user_id, action, entity_type, entity_id, new_value)
        VALUES (${admin_id}, 'evaluator_suspended', 'user', ${evaluator_id}, 'suspended by SP admin')
      `;
      return NextResponse.json({ success: true });
    }

    if (action === "delete_account") {
      // Hard delete — only for pending/unknown accounts with no history
      const hasHistory = await sql`
        SELECT COUNT(*) as count FROM evaluator_session_signups WHERE user_id = ${evaluator_id}
      `;
      if (parseInt(hasHistory[0].count) > 0) {
        return NextResponse.json({ error: "Cannot delete an evaluator with session history. Use suspend instead." }, { status: 400 });
      }
      // Remove membership, auth accounts, user
    ...
    if (action === "reinstate") {
      // Clear late_cancel flags and restore suspended sessions
      await sql`
        UPDATE evaluator_flags SET reviewed = true, reviewed_by = ${admin_id}, reviewed_at = NOW()
        WHERE evaluator_id = ${evaluator_id} AND flag_type = 'late_cancel'
      `;
      await sql`
        UPDATE evaluator_session_signups SET status = 'cancelled'
        WHERE user_id = ${evaluator_id} AND status = 'suspended'
      `;
      await sql`
        INSERT INTO audit_log (user_id, action, entity_type, entity_id, new_value)
        VALUES (${admin_id}, 'evaluator_reinstated', 'user', ${evaluator_id}, 'reinstated by SP admin')
      `;
      return NextResponse.json({ success: true });
    }
