      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-wrap gap-3">
                <div>
                  <h3 className="text-base font-semibold text-gray-900">{phase === "pre_session" ? "Roster ΓÇö Alphabetical" : "Live Rankings"}</h3>
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <label className={`inline-flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-[#1A6BFF] to-[#4D8FFF] text-white rounded-lg text-sm font-semibold cursor-pointer hover:shadow-md ${importing ? "opacity-50" : ""}`}>
                  ...
                  <input type="file" accept=".csv" className="hidden" disabled={importing} onChange={async (e) => {
                    ...
                    const data = await res.json();
                    setUploadMsg(data.success ? `Γ£ô ${data.count} entries uploaded` : "Error: " + data.error);
            ...
            {schedule.length === 0 ? (
              <div className="py-12 text-center bg-white border border-dashed border-gray-200 rounded-xl text-gray-400">
                ...
                <p className="text-sm">No schedule uploaded yet ΓÇö use the Upload CSV button above</p>
              </div>
            ) : (
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {entries.sort((a, b) => (a.group_number || 0) - (b.group_number || 0)).map((e, i) => (
                            <tr key={i} className="hover:bg-gray-50">
                              <td className="px-4 py-2.5 font-medium text-gray-700">{e.group_number ? `Group ${e.group_number}` : "ΓÇö"}</td>
                              <td className="px-4 py-2.5 text-gray-600">{e.scheduled_date?.toString().split("T")[0]}</td>
                              <td className="px-4 py-2.5 text-gray-500">{e.start_time && e.end_time ? `${e.start_time} ΓÇô ${e.end_time}` : "ΓÇö"}</td>
                              <td className="px-4 py-2.5 text-gray-500">{e.location || "ΓÇö"}</td>
                              <td className="px-4 py-2.5 text-gray-500">{sess?.session_type === "testing" ? 0 : (e.evaluators_required || 4)}</td>
                              <td className="px-4 py-2.5">{e.checkin_code ? <CopyCode code={e.checkin_code} scheduleId={e.id} /> : <span className="text-gray-300 text-xs">ΓÇö</span>}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
          <div style={{background:"#fff",borderRadius:"16px",padding:"28px",width:"100%",maxWidth:"480px",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
            <h3 style={{margin:"0 0 4px",fontSize:"16px",fontWeight:"600",color:"#111"}}>Assign Volunteers ΓÇö Session {volunteerModal.sessionNum}</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {athletes.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400 text-sm">No athletes yet ΓÇö upload a CSV above</td></tr>
                  ) : athletes.map((a, i) => (
                    <tr key={a.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-gray-400 text-xs">{i + 1}</td>
                      <td className="px-4 py-3 font-medium text-gray-900">{a.last_name}, {a.first_name}</td>
                      <td className="px-4 py-3 text-gray-500 font-mono text-xs">{a.external_id || "ΓÇö"}</td>
                      <td className="px-4 py-3">{a.position ? <span className={`text-xs px-2 py-0.5 rounded font-medium capitalize ${POSITION_COLORS[a.position] || "bg-gray-100 text-gray-600"}`}>{a.position}</span> : <span className="text-gray-300">ΓÇö</span>}</td>
                      <td className="px-4 py-3 text-gray-500">{a.birth_year || "ΓÇö"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === "reports" && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {sessions.map(s => {
                    ...
                    return (
                      <div key={s.id} className="flex items-center justify-between">
                        <span className="text-sm text-gray-700">Session {s.session_number} ΓÇö {s.name}</span>
              ...
              {/* Team Select ΓÇö placeholder until team generation is built */}
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <div className="flex items-center gap-3 mb-3">
