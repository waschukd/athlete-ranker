...
function AcceptInviteForm() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/director/accept-invite?token=${token}`)
      .then(r => r.json())
      .then(data => {
  ...
  const onSubmit = async (e) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/director/accept-invite", {
        method: "POST",
            ...
            <>
              <div className="mb-6">
                ...
                <p className="text-gray-500 text-sm mb-3">
                  You've been invited as director for <strong>{invite?.category_names?.length}</strong> categor{invite?.category_names?.length === 1 ? "y" : "ies"} at <strong>{invite?.org_name}</strong>. Set a password to get started.
                </p>
                {invite?.category_names?.length > 0 && (
                  <ul className="text-sm text-gray-600 list-disc pl-5 space-y-0.5">
                    {invite.category_names.map(n => <li key={n}>{n}</li>)}
                  </ul>
                )}
              </div>
...
export default function DirectorAcceptInvitePage() {
  return (
