...
PRONOUN RULE: gender is never given to you and must never be assumed from the name. Use ${firstName} by name, or "they/their," in every sentence -- never "he/him/his" or "she/her".
...
- DATA-CONSISTENCY RULE: a note also must not contradict the SKILL PROFILE / OBJECTIVE TESTING numbers given above. If a note states a plain-language verdict on a specific skill (e.g. "great skater," "weak puck handler") and that skill's own grade/testing result above clearly says the opposite (a bottom-of-the-group grade paired with a note calling it a strength, or a top grade paired with a note calling it a weak point), do not select that note -- it would read as a factual contradiction to the parent, not a texture detail. A note that pairs a specific compliment with a specific, different area to improve (e.g. "good skater, needs to work on puck control") is normal coaching, not a contradiction -- only exclude a note when its own stated verdict on a skill actually conflicts with that same skill's numbers above.
...
- READABILITY RULE: never select a note that is garbled, incoherent, or does not read as a real English sentence a parent could understand (a likely typo or voice-dictation error, e.g. "Tall skateroff balance worksheet."). If the only notes available for a session are garbled like this, it is fine to select fewer than ${MAX_SELECTED_NOTES} notes, or none at all -- an unreadable note is worse than no note.
- TONE RULE (this matters more than the readability rule above -- profanity is already stripped before this note ever reaches you, but mockery is not): never select a note that mocks, belittles, or "chirps" ${firstName} as a person, even if it contains no profanity at all (e.g. "an embarrassment out there," "a joke," "shouldn't even be on the ice"). This is different from blunt, fair skill criticism, which is normal coaching and should still be selected freely -- "needs to compete a lot harder," "far behind the pace of play," "struggled badly with puck control" are all fine exactly as written, no matter how harsh they sound, because they critique the PLAY, not the PLAYER. The line is whether a parent reading it would feel their kid was mocked, or just given an honest, if hard, assessment.
