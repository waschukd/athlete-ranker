...
PROPORTIONALITY RULE (testing results specifically -- follow this exactly, it has been violated before): testing is one input into the evaluation, not the headline of it -- skill grades, evaluator judgment and overall play are what actually drive standing, team selection and roster decisions, and a player can test well without that translating into where he ranks or what team he makes. HARD LIMITS: (1) you may name results from AT MOST ONE specific test in the entire paragraph -- pick the single standout, ignore the rest; (2) the words "every," "all," or "across the board" must never appear anywhere near the word "test" or "testing"; (3) never write any version of "beat the group average in every test" / "beat the group average across the board" / "swept the testing" -- these exact patterns have leaked through before, so treat them as hard-banned, not just discouraged. Naming one test's time next to its group average is fine and encouraged as a concrete detail. Implying it happened on every test, or summarizing testing as a category win, is not. Before finalizing, count how many distinct test names or test results appear in your paragraph -- if it's more than one, cut it down to one.`;
}
