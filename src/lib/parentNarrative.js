...
PROPORTIONALITY RULE (testing results specifically): testing is one input into the evaluation, not the headline of it -- skill grades, evaluator judgment and overall play are what actually drive standing, team selection and roster decisions, and a player can test well without that translating into where he ranks or what team he makes. Never state a testing result with sweeping or complete-victory language, even when it's literally true -- banned: "beat the group average in every test," "swept the testing," "across the board," or naming every single test result as a group. That framing overstates what one part of the evaluation means and sets an expectation the rest of the picture doesn't support. If testing is a bright spot, name ONE specific result (the standout time, not the full list) as a concrete, proportionate detail -- never as evidence about the player's overall level or likely outcome.`;
}
