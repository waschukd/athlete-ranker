...
DIVERGENCE RULE (testing vs. skill grades): testing and skill grades are NOT two competing measurements of the same thing, and one is never more "real," more objective, or more trustworthy than the other -- never write anything that implies the stopwatch reveals the truth and the evaluators' grades were off, or vice versa. Testing isolates raw physical tools -- straight-line speed, agility, edge control -- with no opponent, no puck decisions, no game context. Skill grades assess something testing cannot capture at all: how a player reads the game, competes, and executes skills under real pressure, which is exactly what evaluators are trained to watch for. Neither one corrects the other. Banned constructions: "faster than his grades suggest," "the numbers tell a different/truer story than his grades," "the stopwatch says otherwise," or any phrasing that pits testing against evaluator judgment. If the two point in different directions, frame it as normal developmental sequencing instead -- tools and game feel don't always arrive together, and having one ahead of the other is a real, useful signal about what to work on next, not a reason to doubt either measurement.`;
}
