// Demo Soc3: one association with TWO U11 categories — U11 AA (tournament /
// round_robin, to demo schedule-building + AA cuts) and U11 House (standard).
// Full rosters, schedules with groups, and TWO scored sessions (testing +
// scrimmage) for every athlete. Viewed as super admin via God Mode jump.
//   node scripts/seed-demo-soc3.mjs
...
for (const p of prior) {
  ...
  for (const c of cats) {
    ...
    await sql`DELETE FROM scrimmage_teams WHERE age_category_id=${c.id}`.catch(() => {});
...
const [org] = await sql`INSERT INTO organizations (name, type, contact_email) VALUES (${ORG_NAME}, 'association', ${"demosoc3@demo.sidelinestar.com"}) RETURNING id`;
seed ^= org.id * 2654435761;

const FIRST = ["Ella","Mia","Olivia","Ruby","Isla","Lily","Grace","Sophia","Chloe","Zoe","Ava","Emma","Lucas","Liam","Noah","Ethan","Mason","Logan","Jack","Owen","Nora","Ivy","Maya","Leah","Aria","Ari","Cole","Reid","Finn","Beau","Jude","Rhys","Kai","Ren","Nash","Tate","Wren","Sage","Bo","Rex"];
const LAST = ["Boyd","Chan","Doyle","Flynn","Fraser","Grant","Hale","Kerr","Nash","Patel","Sutton","Walsh","Ward","Reed","Cole","Price","Shaw","Dean","Frost","Vance","Webb","Cross","Blair","Hunt","Rowe","Pike","Bell","Fox","Snow","Rae","Lark","Vale","Moss","Bird","Wolfe","Sloan","Quinn","Dale","Peck","Ash"];
const pem = (f, l) => `${f}.${l}@example.com`.toLowerCase();
const SKATER = ["Skating","Puck Skills","Effort / Compete","Hockey IQ"];

async function seedCategory(name, format, N) {
  const [cat] = await sql`
  ...
  await bulk("category_sessions", ["age_category_id","session_number","name","session_type","weight_percentage","status"], [
    { age_category_id:CAT, session_number:1, name:"Session 1", session_type:"testing",   weight_percentage:10, status:"complete" },
    { age_category_id:CAT, session_number:2, name:"Session 2", session_type:"scrimmage", weight_percentage:30, status:"complete" },
  ...
  const abil = new Map(); for (const s of A) abil.set(s.id, clamp(gauss(6.3,1.2),3.4,9.4));
  const catScores = [], drillRows = [], testRows = [];
  for (const s of A) { // session 2 scrimmage scores
    const shift = gauss(0,0.22);
    for (const cid of skaterCatIds) for (const ev of evs) catScores.push({ athlete_id:s.id, age_category_id:CAT, session_number:2, evaluator_id:ev, scoring_category_id:cid, score: clamp(half(abil.get(s.id)+shift+gauss(0,0.5)),1,SCALE), scored_via:"manual" });
  }
  { // session 1 testing
  ...
  await bulk("testing_drill_results", ["athlete_id","age_category_id","session_number","overall_rank"], drillRows);
  await bulk("testing_results", ["athlete_id","age_category_id","session_number","test_name","value","test_rank"], testRows);
  return { CAT, n: A.length };
}

const aa = await seedCategory("U11 AA", "round_robin", 34);
const house = await seedCategory("U11 House", "standard", 30);

console.log("✅ Demo Soc3 ready");
console.log(`   org=${org.id}`);
console.log(`   U11 AA   (tournament) cat=${aa.CAT}  ${aa.n} skaters · sessions 1-2 scored`);
console.log(`   U11 House (standard)  cat=${house.CAT}  ${house.n} skaters · sessions 1-2 scored`);
console.log("");
console.log(`   View (as super admin / God Mode jump):`);
console.log(`   U11 AA:    /association/dashboard/category/${aa.CAT}?org=${org.id}`);
console.log(`   U11 House: /association/dashboard/category/${house.CAT}?org=${org.id}`);
console.log(`   Association: /association/dashboard?org=${org.id}`);
